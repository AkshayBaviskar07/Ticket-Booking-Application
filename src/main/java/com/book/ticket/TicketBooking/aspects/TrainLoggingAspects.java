...
@Component
@Aspect
public class TrainLoggingAspects {
    
    private static final Logger LOGGER = LoggerFactory.getLogger(TrainLoggingAspects.class);

    //    Aspects for controllers

    @Before("execution(* com.book.ticket.TicketBooking.controller.TrainController.*(..))")
    public void logCallBeforeController(JoinPoint point){
        LOGGER.info(CYAN + "Controller Before : "+point.getSignature().getName() + RESET);
    }

    @After("execution(* com.book.ticket.TicketBooking.controller.TrainController.*(..))")
    public void logCallAfterController(JoinPoint point){
        LOGGER.info(PURPLE + "Controller After : "+point.getSignature().getName() + RESET);
    }

    @AfterThrowing("execution(* com.book.ticket.TicketBooking.controller.TrainController.*(..))")
    ...
    @AfterReturning("execution(* com.book.ticket.TicketBooking.controller.TrainController.*(..))")
...
//    Aspects for service
    ...
    @AfterThrowing("execution(* com.book.ticket.TicketBooking.service.TrainService.*(..))")
